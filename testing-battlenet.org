...
#+TITLE: \textnormal{Testing Battle.net}
#+BEAMER_HEADER: \subtitle{(Before deploying to millions of players)}
#+DATE: \textnormal{24th September 2015}
# +AUTHOR: Ben Deane
...
#+CREATOR: Emacs 25.0.50.2 (Org mode 8.3.1)
#+LINK_UP:
#+LINK_HOME:
#+XSLT:
#+LaTeX_CLASS: beamer
...
* Cover notes                                                      :B_noteNH:
:PROPERTIES:
:BEAMER_env: noteNH
:END:
- This talk is about my experiences trying to make my code less likely to fail
- This talk is a compendium of techniques: I think tech progresses best when it
  can draw from a melting pot of techniques.
- Testing is not so well established in C++ as in other languages.
- But here are some techniques that I found useful.
- Really solving 2 problems: testing legacy code and testing for scalability.
...
- composition vs inheritance, value semantics, generics
- manual memory mgmt vs smart ptrs
- callback chains vs coroutines
- low data parallelism, high function concurrency, process level parallelism
...
- Applying unit tests to UTF8 conversion convinced me that they were useful
...
- Well architected is not the same as easy to test
...
- Use the split of logic vs interactions
  ...
  - Base has as few dependencies as possible
...
- cf Planning for failure, servers coming back up
...
- Config injected, but when logic is separate, defaults are easy to provide
...
Even with DI, classes are \emph{onerous to test} unless constructors take few arguments, using narrow interfaces.\\[2\baselineskip]
Corollary: construct with sensibly-configured defaults.
\end{center}
...
- empirical method
- explain
- O(n) algo example omits lower order terms
...
- constants need to be big enough to elicit the required effect
  - make smaller terms fade into noise
...
:END:
\begin{scriptsize}
\texttt{static T generate(size_t g, unsigned long int seed)\\
\string{\\
\ \ switch (g)\\
\ \ \string{\\
...
\ \ v.reserve(n);\\
\ \ std::generate_n(\\
\ \ \ \ std::back_inserter(v), \hilight{n}, [\&] () \string{\\
\ \ \ \ \ \ \hilight{return Arbitrary<ValT>::generate(g, seed++);} \string});\\
\ \ return v;\\
\string}\\[1\baselineskip]}
\end{scriptsize}
...
\ \ \string{\\
\ \ \ \ return std::vector<T>\string{\string};\\
\ \ \string}\\
\string};\\[1\baselineskip]}
\end{scriptsize}
...
:END:
\begin{scriptsize}
\texttt{static T \hilight{generate}(size_t g, unsigned long int seed)\\
\string{\\
\ \ using ValT = typename T::value_type;\\
\ \ T v;\\
\ \ \hilight{size_t n = 10 * ((g / 100) + 1);}\\
...
\ \ std::generate_n(\\
\ \ \ \ std::back_inserter(v), \hilight{n}, [\&] () \string{\\
\ \ \ \ \ \ return \hilight{Arbitrary<ValT>::generate}(g, seed++); \string});\\
\ \ return v;\\
\string}\\[1\baselineskip]}
\end{scriptsize}
...
:END:
\begin{scriptsize}
\texttt{static T \hilight{generate_n}(size_t g, unsigned long int seed)\\
\string{\\
\ \ using ValT = typename T::value_type;\\
\ \ T v;\\
...
\ \ v.reserve(g);\\
\ \ std::generate_n(\\
\ \ \ \ std::back_inserter(v), \hilight{g}, [\&] () \string{\\
\ \ \ \ \ \ return \hilight{Arbitrary<ValT>::generate_n}(g, seed++); \string});\\
\ \ return v;\\
\string}\\[1\baselineskip]}
\end{scriptsize}
...
** The future?
- \texttt{Arbitrary} is a kind of lightweight fuzz testing?
...
** Battle.net is still highly...                                    :B_note:
:PROPERTIES:
:BEAMER_env: note
:END:
- Distributed
...
:END:
\begin{center}
People see this, and say...\\[1\baselineskip]
\hrule
\end{center}
\begin{scriptsize}
\texttt{DEF_PROPERTY(TestName, Suite, \hilight{const string\& s})\\
...
\string{\\
\ \ std::vector<std::pair<T1, T2>{}> ret;\\[1\baselineskip]
\ \ {\color{sh_comment}\textit{// shrink the first}}\\
\ \ auto first_v = \hilight{Arbitrary<T1>::shrink(p.first)};\\
\ \ for (auto\&\& e :\ first_v)\\
\ \ \string{\\
\ \ \ \ ret.push_back(\hilight{std::make_pair(e, p.second)});\\
\ \ \string}\\[1\baselineskip]
...
\ \ for (auto\&\& e :\ second_v)\\
\ \ \string{\\
\ \ \ \ ret.push_back(\hilight{std::make_pair(p.first, e)});\\
\ \ \string}\\[1\baselineskip]
\ \ return ret;\\
\string}\\[1\baselineskip]}
\end{scriptsize}
...
\string{\\
\ \ std::vector<std::tuple<Ts...>{}> ret;\\[1\baselineskip]
...
\ \ for (auto\&\& e :\ head_v)\\
\ \ \string{\\
\ \ \ \ ret.push_back(\hilight{tuple_cons(e, tuple_tail(t))});\\
\ \ \string}\\[1\baselineskip]
\ \ ...\\[1\baselineskip]
\ \ return ret;\\
\string}\\[1\baselineskip]}
...
\ \ for (auto\&\& e :\ tail_v)\\
\ \ \string{\\
\ \ \ \ ret.push_back(\hilight{tuple_cons(std::get<0>(t), e)});\\
\ \ \string}\\[1\baselineskip]
\ \ return ret;\\
\string}\\[1\baselineskip]}
\end{scriptsize}
